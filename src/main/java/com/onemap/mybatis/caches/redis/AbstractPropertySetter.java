...
/**
 * Converts a keyed property string in the Config to a proper Java object
 * representation.
 * 
...
abstract class AbstractPropertySetter<T> {
	...
	static {
		try {
			BeanInfo memcachedConfigInfo = Introspector
					.getBeanInfo(RedisConfiguration.class);
			for (PropertyDescriptor descriptor : memcachedConfigInfo
	 ...
	 */
	public AbstractPropertySetter(final String propertyKey,
		...
		if (this.propertyWriterMethod == null) {
			throw new RuntimeException("Class '"
					...
					+ "' doesn't contain a property '" + propertyName + "'");
		}
	 ...
	 *            the {@link RedisConfiguration}
	 */
	public final void set(Properties config,
		...
		try {
			...
			throw new RuntimeException("Impossible to set property '"
					+ propertyName + "' with value '" + value
					+ "', extracted from ('" + propertyKey + "'="
					+ propertyValue + ")", e);
		}
	}
