...
<!-- 列表内容 begin -->
	<div class="container">
		    	<div class="connav">
			       		» <a href="./list">地块管理</a> &gt; 地块列表    	</div>
		    	<!--  主体   start -->
<div class="topselect">
	<form method="get" action="?" id="form1" name="form1"><p  class="domain">
	 地块名称：<input type="text" class="textinput" id="name" name="name" style="width:90px" value="${warehouse.name}"/></p>
...
<c:otherwise>暂无地块！</c:otherwise> </c:choose>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
        <tr>
				...
				<th><span>楼层</span></th>
				<th><span>库区</span></th>
				<th colspan="3"><span>总容量</span></th>
				<th colspan="3"><span>当前容量</span></th>
    			<th><span>操作</span></th>
  			</tr>
    			...
    			<td >${result.id}</td>
    			<td >${result.name}</td>
    			<td >${result.region.warehouse.site.name}</td>
    			<td >${result.region.warehouse.name}</td>
    			<td >${result.region.floor}</td>
    			<td >${result.region.name}</td>
    			<td >${result.coreCapacity}(核心)</td>
    			<td >${result.relatedCapacity}(关联)</td>
    			<td >${result.commonCapacity}(一般)</td>
    			<td >${result.coreCurrentAmount}/${result.coreThreshold}(核心)</td>
    			<td >${result.relatedCurrentAmount}/${result.coreThreshold}(关联)</td>
    			<td >${result.commonCurrentAmount}/${result.coreThreshold}(一般)</td>
    			<td width=100>
