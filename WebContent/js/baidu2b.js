...
var opts = {
	width : 250, // 信息窗口宽度
	height : 0, // 信息窗口高度
	title : "<div><h1>地块信息</h1><div>", // 信息窗口标题
	enableMessage : false
// 设置允许信息窗发送短息
};
...
function initialize() {
	...
	$
			.getJSON(
					"../landblock/site/" + id + "/trails/lifecycle",
					function(responseData) {
						...
						for ( var i = 0; i < len; i++) {
							...
							if (points.length > 0) {
								...
								var _iw = createInfoWindow2(title, i, responseData.data);
...
function createInfoWindow2(title, i, responseData){	
...
//					});
	...
	var content = "<div style='font-size:12px'>地块编号: " + data.blockNumber+"<br>地块面积(亩):"+data.area + "<br>地块类型: "
										...
										+"<a href='../croplifecycle/charts?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+"' target='_blank'>查看作物生长状态一览</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>查看作物生长状态详情</a><br>"
										/*+"<br><h2>查阅详情:</h2>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+"' target='_blank'>播种出苗</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>越冬分蘖</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>返青拔节</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>抽穗扬花</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>灌浆乳熟</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId+ "' target='_blank'>收获</a><br>"*/
										+"</div>";
