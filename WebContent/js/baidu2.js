					...
					function(responseData) {
						...
						for ( var i = 0; i < len; i++) {
							...
							if (points.length > 0) {
								...
								var _iw = createInfoWindow2(title, i, responseData.data,gpsx,gpsy);
...
function createInfoWindow2(title, i, responseData,gpsx,gpsy){	
	...
	//(i);
//	var url = encodeURI('../site/getJson?id=' + data.id);
////	
...
//					});
	...
	var content = "<div style='font-size:12px'>地块编号: " + data.blockNumber+"<br>地块面积(亩):"+data.area
										+ "<br>地块类型: "
										+ data.blockType.blockType
										+ "<br>土壤类型:"+data.landType.landType
										+"<br>所属合作社:"+data.site.cooperativeName+"<br><br>"
										+"<a href='../managementrecord/charts?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId_mr+ "&x="+gpsx+"&y="+gpsy+"' target='_blank'>查看作物生产过程回溯</a><br>"
										+"<a href='../croplifecycle/chart?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId_lc+"&x="+gpsx+"&y="+gpsy+"' target='_blank'>查看作物生长状态图表</a><br>"
										+"<a href='../yieldprediction/charts?blockId="+ data.id+ "&cropTypeId="+data.cropTypeId_yp+ "&x="+gpsx+"&y="+gpsy+"' target='_blank'>查看作物产量预测图表</a><br>"
										+"</div>";
